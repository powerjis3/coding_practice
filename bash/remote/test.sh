...
## OS check ##
OS_ISSUE=$(sshpass -p $PASSWORD ssh -o ConnectTimeout=2 $USER_ID@$IP  -no StrictHostKeyChecking=no "cat /etc/issue | awk '{print \$1}' | sed -n 1p | tr '[A-Z]' '[a-z]'" 2>&1)
OS_VERSION=$(sshpass -p $PASSWORD ssh -o ConnectTimeout=2 $USER_ID@$IP  -no StrictHostKeyChecking=no "cat /etc/redhat-release | sed 's/[a-z,A-Z]//g' | awk '{print \$1}'" 2>&1)
...
CENTOS_GROUP_CHECK=$(sshpass -p $PASSWORD ssh $USER_ID@$IP  -no StrictHostKeyChecking=no  "echo $PASSWORD | sudo -S cat /root/tmp/group | grep wheel | grep $4 | wc -l" 2>/dev/null)
UBUNTU_GROUP_CHECK=$(sshpass -p $PASSWORD ssh $USER_ID@$IP  -no StrictHostKeyChecking=no  "echo $PASSWORD | sudo -S cat /root/tmp/group | grep sudo | grep $4 | wc -l" 2>/dev/null)

## user add ##
SSH_ALLOW_COMMEND="echo $PASSWORD | sudo -S sed -ie '/^AllowUsers/ s/$/ $4@$5/' /root/tmp/sshd_config"
SSH_ALLOW_ADD="sshpass -p $PASSWORD ssh -o ConnectTimeout=2 $USER_ID@$IP  -no StrictHostKeyChecking=no $SSH_ALLOW_COMMEND"
SUDO_GROUP_ADD="echo $PASSWORD | sudo -S sed -ie '/^sudo:/ s/$/,$4/' /root/tmp/group"
